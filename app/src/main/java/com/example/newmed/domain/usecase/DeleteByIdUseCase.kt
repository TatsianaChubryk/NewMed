package com.example.newmed.domain.usecase

import com.example.newmed.data.reposotiry.PatientRepository
import com.example.newmed.presentation.interfaces.DeleteByIdInterface
import com.example.newmed.presentation.interfaces.PatientInterface

class DeleteByIdUseCase(private val repository: PatientRepository): DeleteByIdInterface {
    override suspend fun patientDeleteClick(id: Int) {
        repository.deletePatientById(id)
    }
}


/*
class DeleteByIdUseCase(private val patientRepository: PatientInterface) {
    suspend fun patientDeleteClick(id: Int) {
        patientRepository.deletePatientById(id)
    }
}*/
